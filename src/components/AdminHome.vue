    ...
    <el-col :span="12" :offset="6">
      ...
      <el-main class="chatWindow">
        <el-row :gutter="20">
          <el-col :span="12" :offset="6">
            <div class="message">
              <div v-for="(item,index) in messageList" :key="item.time">
                <div v-if="item.chatWho==0" style="float: right; width:300px; height:80px">
                ...
                <br />
                <div v-if="item.chatWho==1" style="float: left; width:300px; height:80px">
                    <div style="font-size: 5px;float: left; color:rgb(64, 158, 255)">
                      用户：{{ new Date(item.time).toLocaleTimeString()}}
                    </div><br>
      ...
      <div class="block">
        ...
        <table>
          <div v-for="item in chats" :key="item.chatTime">
            <div v-if="item.chatWho==0 && item.del!=2" style="float: right; width:500px; height:80px">
            ...
            <div v-if="item.chatWho==1" style="float: left; width:500px; height:80px">
                <div style="font-size: 5px;float: left; color:rgb(64, 158, 255)">
...
export default {
  ...
  created() {
    ...
    this.merchantId = localStorage.getItem("websocket_merchantId");
    //如果userId不存在返回到登录页面
    if (!this.merchantId) {
      this.$router.push({
        name: "Login",
  ...
  methods: {
    webSocketInit() {
      ...
      const webSocketUrl = "ws://localhost:8088/websocket/0/" + this.merchantId;
    ...
    handleSendButton() {
      ...
      console.log(this.userId);
      this.addChatForm.userId = this.userId;
    ...
    handleLogoutButton() {
      localStorage.removeItem("websocket_merchantId"); //清除userId然后断开连接
